...
def objective(trial, data_details, train_loaders, val_loaders, test_loaders, layer_dict, args):
    ...
    gnn2 = Modular_GNN(**model_args) #it's probably best to freeze one and ema update :(
    ...
    run = wandb.init(
        ...
        project="qbam-donor", 
        name=f"Merge {layer}, LR{config["lr"]:.5f}", 
        config=config
    )
...
def objective_pretrain(trial, train_loaders, val_loaders, test_loaders, args):
    #build models
    gnn1, model_args = load_model(Modular_GNN(**model_args), args.model_path)
    gnn2, _          = load_model(Modular_GNN(**model_args), args.model_path)
    ...
    config = {**model_args, **opt_args, **config}
    print(f"###############################################################################\n"
            f"{model_args["num_gcn"]} {model_args["gnn_layer"]} Layers\t| {model_args["hidden_channels"]} units\n"
            f"{model_args["num_dense"]} Dense Layers\t| {model_args["dense_hidden"]} units\n"
            f"Dropout Rate: {model_args["dropout_p"]}\n"
            f"Learning Rate: {config["lr"]} with Decay {config["lr_decay"]} and Weight Decay: {config["weight_decay"]}\n"
            f"###############################################################################")
    ...
    run = wandb.init(
        entity="bumjin_joo-brown-university", 
        project="qbam-donor", 
        name=f"Merge {model_args["gnn_layer"]} Pretrain, LR{config["lr"]:.5f}", 
        config=config
    )
...
def main(args):
    ...
    if args.model_path:
        study.optimize(lambda trial: objective_pretrain(trial, train_loaders, val_loaders, test_loaders, args), n_trials=100)
    else:
